<template>
  <div>
    <topbar2 :name="studentMessage.name"/>
    <div class="courses">
      <div class="handle">
        <!-- 查询 -->
        <div class="menu">
...
export default {
  ...
  methods: {
    ...
    deleteCourse(index, row) {
       let strData = {
        studentId:this.studentMessage.studentId,
        courseId:row.Cid
      }
    ...
    selectCourse(row) {
      ...
      this.selectForm.studentId=this.studentMessage.studentId;
    ...
    getstudentcourses(){
       ...
       this.$axios.get("http://localhost:8004/Enrollments/"+this.studentMessage.number).then(res => {
         if(res.status == 200){
